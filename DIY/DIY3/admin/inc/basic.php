...
function to7bit($text,$from_enc="UTF-8") {
	if (function_exists('mb_convert_encoding') && false) { // mb_convert_encoding is deprecated, don't use this - UON Custom Version
   		$text = mb_convert_encoding($text,'HTML-ENTITIES',$from_enc);
    } else {
		$text = htmlspecialchars_decode(iconv('UTF-8', 'ISO-8859-1', htmlentities($text, ENT_COMPAT, 'utf-8', false)));
	}
    $text = preg_replace(
        array('/&szlig;/','/&(..)lig;/',
             '/&([aouAOU])uml;/','/&(.)[^;]*;/'),
